...
# ==================================================
    ...
    # ----------------------------------------------------------------
       ...
       # ==========================================================
        # SECTION 1 : RECHERCHE & CONSULTATION (Version 13k+ étudiants)
        # ==========================================================
        with st.expander("🔍 Consultation par Étudiant ou Salle", expanded=False):
            c_search1, c_search2 = st.columns(2)
            
            with c_search1:
                # On utilise un champ texte pour filtrer par nom ou ID afin d'éviter de charger 13k lignes d'un coup
                search_term = st.text_input("Tapez le NOM ou l'ID de l'étudiant", "").strip()
                
                # On ne lance la recherche que si l'utilisateur a tapé au moins 2 caractères
                if len(search_term) >= 2:
                    # On modifie la requête pour chercher dynamiquement (il faut que votre db_select supporte l'opérateur 'ilike' ou 'or')
                    # Si votre db_select est basique, on récupère les matchs via un filtre spécifique
                    query_etu = db_select("etudiants", "id, nom, prenom", limit=50) # On limite à 50 résultats affichés
                    
                    # Filtrage manuel sur les 13k si vous ne voulez pas changer la fonction db_select
                    # Note: Pour 13k, le mieux est de filtrer côté SQL. 
                    # Voici une approche compatible avec votre structure actuelle :
                    matches = [e for e in query_etu if search_term.lower() in e['nom'].lower() or search_term in str(e['id'])]
                    
                    if matches:
                        dict_etu = {f"{e['nom']} {e['prenom']} (ID: {e['id']})": e['id'] for e in matches}
                        choix_etu_label = st.selectbox("Résultats trouvés", list(dict_etu.keys()))
                        id_etu = dict_etu[choix_etu_label]
                    else:
                        st.warning("Aucun étudiant trouvé.")
                        id_etu = None
                else:
                    st.info("Saisissez au moins 2 caractères pour chercher.")
                    id_etu = None
            
            with c_search2:
                # Les salles sont généralement moins nombreuses, on peut garder le selectbox
            ...
            if id_etu:
                ...
                if mod_ids:
                    # On récupère les examens correspondants aux modules de l'étudiant
                    ...
                    if etu_exams:
                        ...
                        for ex in etu_exams:
                            ...
                            res_etu.append({
                                ...
                                "Date & Heure": ex.get('date_heure'),
                                "Durée": f"{ex.get('duree_minutes')} min",
                                "Statut": "Validé" if ex.get('validated') else "En attente"
                            })
                        st.table(res_etu)
                    else:
                        st.info("Aucun examen trouvé pour cet étudiant.")
                else:
                    st.warning("Cet étudiant n'est inscrit à aucun module.")
